...
- **单条路径最大点数:** 10000 个点
- **路径数量限制:** 最多 100 条路径
- **路径简化阈值:** 使用 Douglas-Peucker 算法，容差 2 像素

#### 历史记录
- **最大历史记录数:** 50 条
- **历史记录大小:** 单个状态对象不超过 2MB
- **自动清理:** 超出限制时删除最旧的记录
...
#### 图片尺寸
- **支持的最大尺寸:** 4096x4096 像素
- **推荐尺寸:** 800x600 到 2048x2048
- **最小尺寸:** 100x100 像素

#### 蒙版数据
- **Base64 大小:** 最大 10MB
- **坐标点数量:** 单次传输最多 10000 个点
- **序列化格式:** JSON，压缩后建议 < 1MB

### 12.4 持久化边界

#### 本地存储
- **存储大小限制:** 单个会话状态不超过 5MB
- **存储键数量:** 最多 100 个键
- **自动清理:** 30 天未使用的数据自动删除
