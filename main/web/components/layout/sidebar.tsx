...
export function Sidebar() {
  ...
  return (
    <div
      className={cn(
        'border-r bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 h-full flex flex-col transition-all duration-300',
        sidebarOpen ? 'w-72' : 'w-16'
      )}
    >
      {/* 头部：新建会话按钮 */}
      <div className={cn('border-b', sidebarOpen ? 'px-2 py-2' : 'px-2 py-2')}>
        <div className={cn('flex items-center', sidebarOpen ? 'justify-between' : 'flex-col gap-2')}>
          ...
          <div
            className={cn(
              ...
              sidebarOpen ? 'px-2 py-2 flex-1 gap-2' : 'p-2 justify-center'
            )}
          ...
          >
            ...
            {sidebarOpen && <div className="text-sm font-semibold">发起新对话</div>}
          </div>
          {/* 折叠/展开按钮 */}
          <Button
            variant="ghost"
      ...
      <ScrollArea className="flex-1">
        <div className={cn(sidebarOpen ? 'p-2' : 'p-1')}>
          {isLoading ? (
            <div className="flex items-center justify-center py-8">
              {sidebarOpen && <div className="text-sm text-muted-foreground">加载中...</div>}
            </div>
          ) : sessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <MessageSquare className={cn('text-muted-foreground/50 mb-3', sidebarOpen ? 'h-12 w-12' : 'h-6 w-6')} />
              {sidebarOpen && (
                <>
